...
/// The one response header this server sets.
///
/// `Header::from_bytes` is fallible because it parses arbitrary bytes, but both
/// arguments here are ASCII constants fixed at compile time, so it cannot fail
/// in practice. Returning an Option rather than unwrapping keeps the server
/// free of any panic path at all: a header that somehow failed to build would
/// cost the content type on one response, not the thread serving every request.
fn json_header() -> Option<Header> {
    Header::from_bytes("Content-Type", CONTENT_TYPE_JSON).ok()
}
        ...
        for request in server.incoming_requests() {
            ...
            if requires_auth(path) {
                ...
                if !auth_ok {
                    ...
                    let mut r = Response::from_string(body).with_status_code(401);
            ...
            let (body, status) = match response {
                Ok(body) => (body, 200),
                Err((status, msg)) => (serde_json::json!({ "error": msg }).to_string(), status),
            };
